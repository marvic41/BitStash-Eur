...
*Setup instructions for a Gitian build of BitStash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitStash
...
- [Building BitStash Core](#building-bitstash-core)
...
Clone the git repositories for bitstash and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitstash/bitstash --recursive
git clone https://github.com/bitstash/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/bitstash/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the bitstash repository under 'Fetch and create inputs' to install sources which require
...
To build BitStash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitstash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitstash/.git/
...
For example:
```bash
URL=https://github.com/laanwj/bitstash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitstash=${COMMIT} --url bitstash=${URL} ../bitstash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitstash=${COMMIT} --url bitstash=${URL} ../bitstash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitstash=${COMMIT} --url bitstash=${URL} ../bitstash/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-bitstash:
./bin/gbuild --commit bitstash=${COMMIT},cpp-eth-bitstash=${ETHCOMMIT} --url bitstash=${URL},cpp-eth-bitstash=${ETHURL} ../bitstash/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the bitstash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitstash/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/bitstash/bitstash-detached-sigs.git

BTCPATH=/some/root/path/bitstash
SIGPATH=/some/root/path/bitstash-detached-sigs

./bin/gbuild --url bitstash=${BTCPATH},signature=${SIGPATH} ../bitstash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitstash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitstash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitstash-osx-build.assert
```
...
[bitstash/gitian.sigs](https://github.com/bitstash/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@bitstash.org) and he will commit them.