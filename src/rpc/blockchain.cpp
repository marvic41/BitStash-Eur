...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // bitstash
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // bitstash
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // bitstash
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // bitstash
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CTxDestination bitstashSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(bitstashSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&bitstashSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of bitstash addresses\n"
