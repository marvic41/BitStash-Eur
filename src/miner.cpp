...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setBitStashSchedule(bitstashDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = bitstashDGP.getBlockSize(nHeight);
    minGasPrice = bitstashDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = bitstashDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    BitStashTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractBitStashTX resultConverter;
    if(!convert.extractionBitStashTransactions(resultConverter)){
    ...
    for(BitStashTransaction bitstashTransaction : bitstashTransactions){
        txGas += bitstashTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + bitstashTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(bitstashTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, bitstashTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "bitstashstake";
...
void StakeBitStashs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
