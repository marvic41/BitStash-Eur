...
#include <bitstash/bitstashtransaction.h>
...
class BitStashState : public dev::eth::State {
    
public:

    BitStashState();

    BitStashState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, BitStashTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
private:
    ...
    dev::Address createBitStashAddress(dev::h256 hashTx, uint32_t voutNumber);
...
struct TemporaryState{
    std::unique_ptr<BitStashState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<BitStashState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(BitStashState* _state, const std::vector<TransferInfo>& _transfers, const BitStashTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const BitStashTransaction& transaction;
