#include <bitstash/bitstashDGP.h>

void BitStashDGP::initDataEIP158(){
...
bool BitStashDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule BitStashDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t BitStashDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BitStashDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BitStashDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BitStashDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BitStashDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BitStashDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BitStashDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BitStashDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BitStashDGP::createParamsInstance(){
...
dev::Address BitStashDGP::getAddressForBlock(unsigned int blockHeight){
...
void BitStashDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BitStashDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BitStashDGP::parseStorageOneUint64(uint64_t& value){
...
void BitStashDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BitStashDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
