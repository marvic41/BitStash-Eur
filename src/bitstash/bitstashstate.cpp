...
#include <bitstash/bitstashstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

BitStashState::BitStashState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BitStashState::openDB(_path + "/bitstashDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BitStashState::BitStashState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BitStashState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BitStashTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBitStashAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            bitstash::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> BitStashState::vins() const // temp
{
...
void BitStashState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BitStashState::vin(dev::Address const& _a) const
{
    return const_cast<BitStashState*>(this)->vin(_a);
}

Vin* BitStashState::vin(dev::Address const& _addr)
{
...
// void BitStashState::commit(CommitBehaviour _commitBehaviour)
// {
...
void BitStashState::kill(dev::Address _addr)
{
...
void BitStashState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address BitStashState::createBitStashAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void BitStashState::deleteAccounts(std::set<dev::Address>& addrs){
...
void BitStashState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BitStashState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
