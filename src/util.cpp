...
const char * const BITCOIN_CONF_FILENAME = "bitstash.conf";
const char * const BITCOIN_PID_FILENAME = "bitstashd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitStash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitStash
    // Mac: ~/Library/Application Support/BitStash
    // Unix: ~/.bitstash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitStash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitStash";
#else
    // Unix
    return pathRet / ".bitstash";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BitStash Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitStash Core developers";
    }
