...
#include <qt/bitstashversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    bitstashVersionChecker = new BitStashVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BitStash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitstash: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BitStash addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BitStash addresses"));
    ...
    openAction->setStatusTip(tr("Open a bitstash: URI or payment request"));//SATSH_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitStash command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && bitstashVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(SATSH_RELEASES, SATSH_RELEASES);
            QString message(tr("New version of BitStash wallet is available on the BitStash source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to BitStash network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("BitStash"); // default title
