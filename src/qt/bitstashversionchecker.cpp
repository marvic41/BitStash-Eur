...
#define paternVersion "bitstash-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

BitStashVersionChecker::BitStashVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

BitStashVersionChecker::~BitStashVersionChecker()
{

}

bool BitStashVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> BitStashVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(SATSH_RELEASES)));
    QEventLoop event;
...
Version BitStashVersionChecker::getMaxReleaseVersion()
{
