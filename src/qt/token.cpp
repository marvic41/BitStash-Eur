...
bool ToHash160(const std::string& strBitStashAddress, std::string& strHash160)
{
    CTxDestination bitstashAddress = DecodeDestination(strBitStashAddress);
    if(!IsValidDestination(bitstashAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&bitstashAddress);
    if(keyid){
...
bool ToBitStashAddress(const std::string& strHash160, std::string& strBitStashAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination bitstashAddress = keyid;
    if(IsValidDestination(bitstashAddress)){
        strBitStashAddress = EncodeDestination(bitstashAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBitStashAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBitStashAddress(tokenEvent.receiver, tokenEvent.receiver);
