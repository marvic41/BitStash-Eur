...
#include <bitstash/storageresults.h>


extern std::unique_ptr<BitStashState> globalState;
...
using ExtractBitStashTX = std::pair<std::vector<BitStashTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // bitstash
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashBitStashRoot,
    const dev::h256& oldHashStateRoot, const std::vector<BitStashTransaction>& transactions);
...
class BitStashTxConverter{

public:

    BitStashTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionBitStashTransactions(ExtractBitStashTX& bitstashTx);
...
private:
    ...
    BitStashTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<BitStashTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
