...
inline void initState(){
    ...
    const std::string dirBitStash = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<BitStashState>(new BitStashState(dev::u256(0), BitStashState::openDB(dirBitStash, hashDB, dev::WithExisting::Trust), dirBitStash + "/bitstashDB", dev::eth::BaseState::Empty));
...
inline dev::Address createBitStashAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline BitStashTransaction createBitStashTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    BitStashTransaction txEth;
    if(recipient == dev::Address()){
        txEth = BitStashTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = BitStashTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<BitStashTransaction> txs){
