...
void checkResult(bool isCreation, std::vector<BitStashTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    BitStashTxConverter converter(transaction, NULL);
    ExtractBitStashTX bitstashTx;
    BOOST_CHECK(converter.extractionBitStashTransactions(bitstashTx));
    std::vector<BitStashTransaction> result = bitstashTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(bitstashtxconverter_tests, TestingSetup)
