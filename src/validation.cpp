...
#if defined(NDEBUG)
# error "BitStash cannot be compiled without assertions."
#endif
...
std::unique_ptr<BitStashState> globalState;
...
const std::string strMessageMagic = "BitStash Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = bitstashDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = bitstashDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            BitStashTxConverter converter(tx, NULL);
            ExtractBitStashTX resultConverter;
            if(!converter.extractionBitStashTransactions(resultConverter)){
            ...
            std::vector<BitStashTransaction> bitstashTransactions = resultConverter.first;
            std::vector<EthTransactionParams> bitstashETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(BitStashTransaction bitstashTransaction : bitstashTransactions){
                sumGas += bitstashTransaction.gas() * bitstashTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, bitstashTransaction.gasPrice());
                } else {
                    txMinGasPrice = bitstashTransaction.gasPrice();
                }
                VersionVM v = bitstashTransaction.getVersion();
                if(v.format!=0)
                ...
                if(bitstashTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(bitstashTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(bitstashTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += bitstashTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)bitstashTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(bitstashETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > bitstashTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SATSH additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // bitstash
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // bitstash
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    BitStashTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<BitStashTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path bitstashDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(bitstashDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(bitstashDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(BitStashTransaction& tx : txs){
        //validate VM version
...
bool BitStashTxConverter::extractionBitStashTransactions(ExtractBitStashTX& bitstashtx){
    std::vector<BitStashTransaction> resultTX;
    ...
    bitstashtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool BitStashTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool BitStashTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
BitStashTransaction BitStashTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    BitStashTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = BitStashTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = BitStashTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // bitstash
    BitStashDGP bitstashDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setBitStashSchedule(bitstashDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = bitstashDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = bitstashDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = bitstashDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // bitstash
...
///////////////////////////////////////////////////////////////////////////////////////// bitstash
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            BitStashTxConverter convert(tx, &view, &block.vtx);

            ExtractBitStashTX resultConvertBitStashTX;
            if(!convert.extractionBitStashTransactions(resultConvertBitStashTX)){
            ...
            if(!CheckMinGasPrice(resultConvertBitStashTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertBitStashTX.first, blockGasLimit, pindex->pprev);
            ...
            for(BitStashTransaction& qtx : resultConvertBitStashTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertBitStashTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertBitStashTX.first[k].from(), resultConvertBitStashTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // bitstash
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // bitstash
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // bitstash
            globalState->setRootUTXO(oldHashUTXORoot); // bitstash
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // bitstash
        uint32_t sizeBlockDGP = bitstashDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for BitStash PoS we need to sync this after every block to ensure txdb is populated for
