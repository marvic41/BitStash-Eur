...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitstash/bitstash>";
    const std::string URL_WEBSITE = "<https://www.bitstash.io>";
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path bitstashStateDir = GetDataDir() / "stateBitStash";
                bool fStatus = fs::exists(bitstashStateDir);
                const std::string dirBitStash(bitstashStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsBitStashstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<BitStashState>(new BitStashState(dev::u256(0), BitStashState::openDB(dirBitStash, hashDB, dev::WithExisting::Trust), dirBitStash, existsBitStashstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::bitstashMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(bitstashStateDir.string()));
                if (fReset) {
